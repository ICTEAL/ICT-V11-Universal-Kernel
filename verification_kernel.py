import numpy as np

def ict_v11_velocity(radius_kpc, visible_mass_solar):
    """
    ICT V11 Kernel: Calculates orbital velocity using Quartic Screening.
    Target: Milky Way Edge (25 kpc) -> ~230 km/s
    """
    # --- UNIVERSAL CONSTANTS ---
    G_GALACTIC = 4.30e-6  # [cite: 38]
    L_CONST = 1.2e-10     # [cite: 39]
    RHO_CRIT = 1e-22      # [cite: 40]
    C_INDEX = 1.5         # [cite: 41]
    
    KPC_TO_M = 3.086e19   # [cite: 43]
    MSUN_TO_KG = 1.989e30 # [cite: 44]
    KPC_TO_M3 = (KPC_TO_M)**3 # [cite: 45]

    # --- 1. NEWTONIAN COMPONENT ---
    v_newton = np.sqrt((G_GALACTIC * visible_mass_solar) / radius_kpc) # [cite: 47]

    # --- 2. QUARTIC SCREENING ---
    vol_kpc = (4/3) * np.pi * (radius_kpc**3) # [cite: 49]
    rho_si = (visible_mass_solar / vol_kpc) * (MSUN_TO_KG / KPC_TO_M3) # [cite: 50]
    
    # The Screen: 1 / (1 + (rho/crit)^4)
    screen = 1.0 / (1.0 + (rho_si / RHO_CRIT)**4) # [cite: 52]

    # --- 3. ICT COMPLEXITY BOOST ---
    r_meters = radius_kpc * KPC_TO_M # [cite: 54]
    v_boost_kms = (np.sqrt(L_CONST * C_INDEX * r_meters) / 1000.0) * screen # [cite: 55]

    return v_newton + v_boost_kms # [cite: 56]

# Verification: Milky Way Test at 25 kpc
vm = ict_v11_velocity(25, 6.0e10) # [cite: 58]
print(f"ICT V11 Prediction at 25 kpc: {vm:.2f} km/s") #
